...
static UIWindow *g_MWKProgressIndicatorWindow = nil;

//#define MWKProgressIndicatorHeight 64.0f
#define MWKProgressIndicatorHeight  20+22.0f
...
+ (instancetype)sharedIndicator
{
    ...
    if (g_MWKProgressIndicatorWindow == nil)
    {
        g_MWKProgressIndicatorWindow = [[UIWindow alloc] initWithFrame:CGRectMake(0, 0, UI_SCREEN_WIDTH, MWKProgressIndicatorHeight)];
        g_MWKProgressIndicatorWindow.windowLevel = UIWindowLevelAlert;
        [g_MWKProgressIndicatorWindow makeKeyAndVisible];
    }
    
    [g_MWKProgressIndicatorWindow addSubview:indicator];//这样做的目的，是修复了在很多页面中，显示提示信息时有可能被其他视图（如状态栏，导航栏）遮住而显示不出来的问题
...
//    [_progressTrack setBackgroundColor:[UIColor colorWithRed:0.53 green:0.82 blue:1 alpha:1]];
     [_progressTrack setBackgroundColor: COLOR_C1];
...
{
    if (CGRectGetMinY(self.frame) == -MWKProgressIndicatorHeight)
    {
        g_MWKProgressIndicatorWindow = nil;
        return;
    }
    ...
    if (animated)
    {
        ...
        [self setTopLocationValue:-MWKProgressIndicatorHeight-2 completion:^(BOOL finished) {
            g_MWKProgressIndicatorWindow = nil;
            
            NSLog(@"g_MWKProgressIndicatorWindow = nil");
        }];
    }
    else
    {
        _progressTrack.frame = CGRectMake(0, 0, 0, self.frame.size.height);
        [self setTopLocationValue:-MWKProgressIndicatorHeight-2 withDuration:0.0 completion:^(BOOL finished) {
            g_MWKProgressIndicatorWindow = nil;
        }];
    }
}
...
- (void)setTopLocationValue:(float)value completion:(void (^)(BOOL finished))completion
{
    [self setTopLocationValue:value withDuration:0.4 completion:completion];
}
...
- (void)setTopLocationValue:(float)value withDuration:(float)duration completion:(void (^)(BOOL finished))completion
{
    [UIView animateWithDuration:duration delay:0.0 usingSpringWithDamping:0.7 initialSpringVelocity:1.0 options:UIViewAnimationOptionCurveEaseOut animations:^
     {
         [self setFrame:CGRectMake(0, value, self.bounds.size.width, self.bounds.size.height)];
...
[[MWKProgressIndicator sharedIndicator] showWithColor:COLOR_C1 duration:2 message:errorMessage];
}
...
//    [[MWKProgressIndicator sharedIndicator] showWithColor:[UIColor greenColor] duration:2 message:successMessage];
    [[MWKProgressIndicator sharedIndicator] showWithColor:COLOR_C1 duration:2 message:successMessage];
}
...
- (void)showWithColor:(UIColor *)color duration:(float)duration message:(NSString *)message
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [self setTopLocationValue:-MWKProgressIndicatorHeight-2 withDuration:hideDuration];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((duration+hideDuration) * NSEC_PER_SEC)), dispatch_get_main_queue(),^
                       {
                           ...
                           [self setTopLocationValue:-MWKProgressIndicatorHeight-2 withDuration:hideDuration completion:^(BOOL finished) {
                               g_MWKProgressIndicatorWindow = nil;
                           }];
