...
{
    _searchTable = [[UITableView alloc] initWithFrame:CGRectMake(0, yPoint, 320, self.view.frame.size.height - yPoint )];
...
//    if (!_unResultView) {
//        _unResultView = [[NoDataView alloc]initWithFrame:CGRectMake(0, yPoint, UI_SCREEN_WIDTH, UI_SCREEN_HEIGHT-yPoint)];
//        _unResultView.noDataImgView = [[UIImageView alloc]initWithFrame:CGRectMake((UI_SCREEN_WIDTH-58)/2, (UI_SCREEN_HEIGHT-yPoint-50)/2, 58, 50)];
//        [self.view addSubview:_unResultView];
//        [_unResultView setHidden:YES];
//    }
}
...
- (void)networkRequestIsPull:(BOOL)isPull
{
    ...
    [dict setObject:[NSNumber numberWithInt:(int)pageSize] forKey:@"pageSize"];
    
    
    [HttpRequest collocationGetRequestPath:nil methodName:@"CollocationFound" params:dict success:^(NSDictionary *dict) {
...
{
    CGFloat originY = self.headView.size.height-40;
    CGRect  noneDataRect = CGRectMake(0, originY, UI_SCREEN_WIDTH, UI_SCREEN_HEIGHT-originY);
    if (!_unResultView) {
        _unResultView = [SUTILITY_TOOL_INSTANCE createLayOutNoDataViewRect:noneDataRect WithImage:NONE_DATA_COLLOCATION andImgSize:CGSizeMake(67, 50) andTipString:@"没有发现" font:FONT_SIZE(18) textColor:[Utils HexColor:0x999999 Alpha:1.0] andInterval:10.0];

    }
    [self.view insertSubview:_unResultView aboveSubview:_foundTable];
...
//    NSInteger row = indexPath.row;
    if ([tableView isEqual:_searchTable]) {
        ...
        FoundCellModel *model = [sourceArray objectAtIndex:indexPath.row];
        MBMyStoreInfoModel * inforModel = [[MBMyStoreInfoModel alloc]init];
        inforModel.userId = model.userId;
