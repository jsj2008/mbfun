...
#define SBlur_IS_IOS8    ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0)

@protocol SBlurGridMenuButtonDelegate <NSObject>

- (void)didTapOnGridMenuItem:(SBlurGridMenuItem *)item;

@end

@interface UIImage (SBlurGridMenu)
...
@interface SBlurGridMenuFlowLayout : UICollectionViewFlowLayout

@end

@interface SBlurGridMenuCell : UICollectionViewCell

@property (nonatomic, strong) SBlurGridMenuItem *menuItem;
...
@property (nonatomic, weak) id <SBlurGridMenuButtonDelegate> delegate;

@end

@interface SBlurGridMenuItem ()

@end

@interface SBlurGridMenu () <SBlurGridMenuButtonDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, strong) UIView *blurView;
@property (nonatomic, strong) NSMutableArray *buttons;
@property (nonatomic, strong) SBlurGridMenuFlowLayout *flowLayout;
@property (nonatomic, strong) UITapGestureRecognizer *backgroundTapGestureRecognizer;

@end

@implementation SBlurGridMenu

- (instancetype)initWithMenuItems:(NSArray *)items {
    self.flowLayout = [[SBlurGridMenuFlowLayout alloc] init];
    
    
    
    if (UI_SCREEN_WIDTH > 375)//6 plus
    {
    ...
    if (self) {
        _blurEffectStyle = SBlurBlurEffectStyleDark;
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[SBlurGridMenuCell class] forCellWithReuseIdentifier:@"GridMenuCell"];
}
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    return self.blurEffectStyle == SBlurBlurEffectStyleDark ? UIStatusBarStyleLightContent : UIStatusBarStyleDefault;
}

#pragma mark - UICollectionView Delegate & DataSource

- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    SBlurGridMenuCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"GridMenuCell" forIndexPath:indexPath];
    SBlurGridMenuItem *item = [self.menuItems objectAtIndex:indexPath.row];
    cell.delegate = self;
...
#pragma mark - SBlurGridMenuItem Delegate
...
@implementation SBlurGridMenuItem

@end

@implementation SBlurGridMenuCell
...
- (void)setupCell {
    ...
    self.circleButton.layer.borderColor = self.blurEffectStyle == SBlurBlurEffectStyleDark?[UIColor whiteColor].CGColor:[UIColor darkGrayColor].CGColor;
    ...
    self.iconView.tintColor = self.blurEffectStyle == SBlurBlurEffectStyleDark?[UIColor whiteColor]:[UIColor darkGrayColor];
    ...
    [self.titleLabel setTextColor:self.blurEffectStyle == SBlurBlurEffectStyleDark?[UIColor whiteColor]:[UIColor darkGrayColor]];
...
- (void)setBlurEffectStyle:(SBlurBlurEffectStyle)blurEffectStyle {
...
- (void)buttonTouchDown:(UIButton *)button {
    if (self.blurEffectStyle == SBlurBlurEffectStyleDark) {
...
#pragma mark - SBlurGridMenuFlowLayout 

@implementation SBlurGridMenuFlowLayout
...
#pragma mark - SBlurGridMenu Categories

@implementation UIViewController (SBlurGridMenu)

@dynamic gridMenu;

- (void)presentGridMenu:(SBlurGridMenu *)menu animated:(BOOL)flag completion:(void (^)(void))completion {
    [menu setModalPresentationStyle:SBlur_IS_IOS8?UIModalPresentationOverCurrentContext:UIModalPresentationCurrentContext];
