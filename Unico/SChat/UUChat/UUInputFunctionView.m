...
- (id)initWithSuperVC:(UIViewController *)superVC
{
    ...
    if (self) {
        ...
        [self.btnVoiceRecord setTitle:@"按住发送语音" forState:UIControlStateNormal];
        [self.btnVoiceRecord setTitle:@"松开确认发送" forState:UIControlStateHighlighted];
...
{
    [UUProgressHUD changeSubTitle:@"放开取消操作"];
}

- (void)RemindDragEnter:(UIButton *)button
{
    [UUProgressHUD changeSubTitle:@"向上滑动取消操作"];
}
...
- (void)endConvertWithData:(NSData *)voiceData
{
    ...
    [UUProgressHUD dismissWithSuccess:@"录制成功"];
...
- (void)failRecord
{
    [UUProgressHUD dismissWithSuccess:@"录制的语音过短"];
...
- (void)sendMessage:(UIButton *)sender
{
    if (self.isAbleToSendTextMessage) {
        ...
        [self.TextViewInput resignFirstResponder];
        UIActionSheet *actionSheet= [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"拍摄",@"相册",nil];
...
-(void)addCarema{
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        //如果没有提示用户
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"您没有允许使用相机" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alert show];
    }
}
