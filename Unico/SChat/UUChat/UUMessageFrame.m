...
- (void)setMessage:(UUMessage *)message{
    ...
    CGFloat idX;
    if (_message.from == UUMessageFromMe) {
        idX = iconX - ChatIconWH;
    }else{
        idX = ChatIconWH*2;
    }
    _nameF = CGRectMake(idX, iconY, ChatIconWH, 15);
...
//    CGFloat contentY = iconY+15+5;
    ...
    switch (_message.type) {
        case UUMessageTypeText:
            contentSize = [_message.strContent sizeWithFont:FONT_t3  constrainedToSize:CGSizeMake(ChatContentW, CGFLOAT_MAX) lineBreakMode:NSLineBreakByWordWrapping];
            contentX = CGRectGetMaxX(_iconF) + 10;
            contentSize.width -= 15 + 5;
