...
- (void)mergeSegmentsUsingPreset:(NSString *)exportSessionPreset
                   withTimeRange:(CMTimeRange)timeRange
                     withYOffset:(float)offsetY
        ...
        if (_segments.count == 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (offsetY != 0)
                {
                    ...
                    NSLog(@"Video Track Time Range: %f %f",
                    ...
                    float min = MIN(clipVideoTrack.naturalSize.height, clipVideoTrack.naturalSize.width);
                    
                    videoComposition.renderSize = CGSizeMake(min, min);

                    NSLog(@"render size: %f %f", clipVideoTrack.naturalSize.width, clipVideoTrack.naturalSize.height);

                    AVMutableVideoCompositionInstruction *instruction = [AVMutableVideoCompositionInstruction videoCompositionInstruction];
                    
                    instruction.timeRange = CMTimeRangeMake(kCMTimeZero, kCMTimeIndefinite);
...
//                        Use this code if you want the viewing square to be in the middle of the video
                        CGAffineTransform t1 = CGAffineTransformMakeTranslation(clipVideoTrack.naturalSize.height, -clipVideoTrack.naturalSize.width * offsetY );
                    ...
                    {
                        finalTransform = CGAffineTransformMakeTranslation(0, -clipVideoTrack.naturalSize.height * offsetY);
                    }
...
- (void)mergeSegmentsUsingPreset:(NSString *)exportSessionPreset
                   withTimeRange:(CMTimeRange)timeRange
                     withYOffset:(float)offsetY
                 heightWidthRatio:(float)heightWidthRatio
                            rotate:(BOOL)rotate
               completionHandler:(void(^)(NSURL *outputUrl, NSError *error))completionHandler
{
    [self dispatchSyncOnSessionQueue:^{
        ...
        if (_segments.count == 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (offsetY != 0)
                {
                    ...
                    videoComposition.renderSize = CGSizeMake(clipVideoTrack.naturalSize.width, clipVideoTrack.naturalSize.width*heightWidthRatio);
                    ...
                    //if (clipVideoTrack.naturalSize.width > clipVideoTrack.naturalSize.height && rotate)//rotate 是否考虑
                    if (clipVideoTrack.naturalSize.width > clipVideoTrack.naturalSize.height && NO)////暂时不考虑旋转
                    {
                        NSLog(@"需要旋转");
                        // 需要旋转
...
- (CGSize)getVideoNaturalSize
{
    if (_segments.count > 0) {
        ...
        AVAssetTrack *clipVideoTrack = [[asset tracksWithMediaType:AVMediaTypeVideo] objectAtIndex:0];
        return clipVideoTrack.naturalSize;
    }
