//
//  SCollocationCollectionViewLayout.m
//  Wefafa
//
...
@interface SCollocationCollectionViewLayout ()
// 所有item的属性的数组
@property (nonatomic, strong) NSArray *layoutAttributesArray;
@end

@implementation SCollocationCollectionViewLayout
/**
...
- (void)computeAttributesWithItemWidth:(CGFloat)itemWidth {
    ...
    //header高度
    NSInteger headerHeight = self.headerViewHeight;
    // 初始化
    ...
    for (LNGood *good in self.goodsList) {
        ...
        // X值
        CGFloat itemX = (itemWidth + self.minimumInteritemSpacing) * column + self.sectionInset.left + self.marginToLetf;
        // Y值
        CGFloat itemY = columnHeight[column]+headerHeight + 10;
        CGFloat itemH = 0;
        // 等比例缩放 计算item的高度
        if (_isNotCalculate) {
            itemH = 240;
        }else{
            itemH = good.h * itemWidth / good.w;
            if (good.show_type == nil || [good.show_type isEqualToNumber:@0] ) {//&& [good.show_type integerValue] <= 0) {
                itemH += 136/2;
            }else if ([good.show_type isEqualToNumber:@1]) {
                itemH += 51;    //banner顶部时间条
            }
        }
    ...
    // 添加页眉属性
    CGSize headerSize = self.footerReferenceSize;
    if (_headerViewHeight > 0 || (headerSize.width > 0 && headerSize.height > 0)) {
        ...
        headerAttr.frame = CGRectMake(0, 0, self.collectionView.bounds.size.width, headerHeight);
