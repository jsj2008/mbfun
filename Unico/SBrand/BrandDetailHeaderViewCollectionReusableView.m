//
//  BrandDetailHeaderViewCollectionReusableView.m
//  Wefafa
//
...
@interface BrandHeaderCollectionViewCell : UICollectionViewCell
...
@implementation BrandHeaderCollectionViewCell
...
@interface BrandDetailHeaderViewCollectionReusableView ()<UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
...
@property (weak, nonatomic) IBOutlet UIView *showHBackView;
...
static NSString *brandDetailCellID = @"brandDetailCellID";

@implementation BrandDetailHeaderViewCollectionReusableView
...
- (void)awakeFromNib {
    ...
    [self.showUserCollectionView registerClass:[BrandHeaderCollectionViewCell class] forCellWithReuseIdentifier:brandDetailCellID];
    ...
    [_showHBackView.layer addSublayer:layer];
    UITapGestureRecognizer *tapGest=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(gotoBrandStory)];
    [_showHBackView addGestureRecognizer:tapGest];
    UITapGestureRecognizer *contentTapGest=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(gotoBrandDaily)];
    _contentImageView.userInteractionEnabled=YES;
    [_contentImageView addGestureRecognizer:contentTapGest];
    
    _brandImg.layer.cornerRadius = _brandImg.width / 2;
    _brandImg.layer.masksToBounds = YES;
    _brandImg.layer.borderColor = COLOR_C9.CGColor;
    _brandImg.layer.borderWidth = 1;
}
-(void)gotoBrandDaily
{
    if ([self.delegate respondsToSelector:@selector(brandHeaderGotoBrandDaily:)]) {
        [self.delegate brandHeaderGotoBrandDaily:self];
    }
}
-(void)gotoBrandStory
{
    if ([self.delegate respondsToSelector:@selector(brandHeaderGotoStoryDetailVC:)]) {
        [self.delegate brandHeaderGotoStoryDetailVC:self];
    }
}
...
- (void)setContentModel:(SBrandStoryDetailModel *)contentModel{
    ...
    [self.brandImg sd_setImageWithURL:[NSURL URLWithString:contentModel.logo_img]];
...
- (UICollectionViewCell*)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    BrandHeaderCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:brandDetailCellID forIndexPath:indexPath];
