...
@interface CBStoreHouseTransitionInteractiveTransition(){
...
- (void)detach{
    [self.parentViewController.view removeGestureRecognizer:edgePanRecognizer];
    edgePanRecognizer = nil;
}

- (void)attachToViewController:(UIViewController*)viewController
{
    // 没有子vc，不挂事件。
    if (viewController.navigationController.childViewControllers.count<=1) {
        return;
    }
    
    self.parentViewController = viewController;
    edgePanRecognizer = [[UIScreenEdgePanGestureRecognizer alloc] initWithTarget:self action:@selector(handleEdgePanRecognizer:)];
