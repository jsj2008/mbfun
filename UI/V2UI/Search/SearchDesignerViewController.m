...
//    [_searchBar setBarStyle:UIBarStyleBlackOpaque];
    ...
    _sourceTable = [[UITableView alloc] initWithFrame:CGRectMake(0, yPoint, self.view.frame.size.width, self.view.frame.size.height - yPoint) style:UITableViewStylePlain];
...
- (void)networkRequestIsPull:(BOOL)isPull
{
    ...
    BOOL request = [[MBShoppingGuideInterface create] requestPostUrlName:@"DesignerSearchFilter" param:dict responseAll:responseDict responseMsg:message];
        ...
        if (request) {
            ...
            if (array.count > 0) {
                for(NSDictionary *dict in array){
                    DesignerModel *model = [[DesignerModel alloc] initWithDesignerInfo:dict];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([sns.ldap_uid isEqualToString:currentModel.userId]) {
        ...
        [[AppDelegate rootViewController] pushViewController:userCenter animated:YES];
    } else {
        OtherPeopleViewController *otherPeop=[[OtherPeopleViewController alloc]initWithNibName:@"OtherPeopleViewController" bundle:nil];
        DesignerModel *designerModel = dataArray[indexPath.row];
        otherPeop.user_ID = designerModel.userId;
...
- (void)attentionDesigner
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        BOOL isSuccess =
        [SHOPPING_GUIDE_ITF requestPostUrlName:@"UserConcernCreate" param:paramDic responseAll:nil responseMsg:returnMessage];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (isSuccess) {
                ...
                if (cell) {
                    [cell.favoriteBtn setImage:[UIImage imageNamed:@"btn_match_selected"] forState:UIControlStateNormal];
                }
