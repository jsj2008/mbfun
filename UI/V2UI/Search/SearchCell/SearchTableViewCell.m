...
- (id)initWithFrame:(CGRect)frame isCollocation:(BOOL)isCollocation
{
    ...
    if (self) {
        ...
        _heartImg = [[UIImageView alloc] initWithFrame:CGRectMake(frame.size.width - 50, _imageView.frame.size.height + 10, 20, 20)];
        [_heartImg setImage:[UIImage imageNamed:@"btn_like_normal@3x.png"]];
        [self addSubview:_heartImg];
        
        _countLabel = [[UILabel alloc] initWithFrame:CGRectMake(_heartImg.frame.origin.x + 20 + 5, _imageView.frame.size.height + 10, 20, 20)];
...
//    [_nowPriceLabel setAttributedText:content];
    [_countLabel setText:[NSString stringWithFormat:@"%d",model.favortieCount]];
}
...
- (void)updateSubViewWithInfo:(SearchCollocationInfo *)model
{
    ...
    [_headImage setImageAFWithURL:[NSURL URLWithString:[dict objectForKey:@"headPortrait"]] placeholderImage:[UIImage imageNamed:@"ico_userprofile_femaleimage@3x.png"]];
    [_nameLabel setText:[dict objectForKey:@"nickName"]];
    [_countLabel setText:[NSString stringWithFormat:@"%d",model.favoriteCount]];
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier isCollocation:(BOOL)isCollocation
{
    ...
    if (self) {
        ...
        leftView = [[SearchCellSubView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width/2-0.25, 169.5) isCollocation:isCollocation];
        [leftView setDelegate:self];
        [self.contentView addSubview:leftView];
        
        rightView = [[SearchCellSubView alloc] initWithFrame:CGRectMake(self.frame.size.width/2+0.5, 0, self.frame.size.width/2-0.5, 169.5) isCollocation:isCollocation];
