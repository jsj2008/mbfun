...
@interface MBSideViewController ()<UIGestureRecognizerDelegate>{
...
@implementation MBSideViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        _leftViewShowWidth = 320-88;
        _rightViewShowWidth = 320-88;
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (_currentView!=_rootViewController.view) {
        ...
        self.overLayView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, self.view.frame.size.width, self.view.frame.size.height)];
        self.overLayView.backgroundColor = [UIColor blackColor];
        self.overLayView.tag = 911;
        self.overLayView.alpha = 0.0;
        
        [_currentView addSubview:self.overLayView];
    }
...
- (void)showLeftViewController:(BOOL)animated{
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        [UIView animateWithDuration:0.3 animations:^{
            self.overLayView.alpha = _showBoundsShadow ? 0.5f : 0.0f;
        }];
...
- (void)hideSideViewController:(BOOL)animated{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.overLayView.alpha = false ? 0.5f : 0.0f;
    }];
    NSTimeInterval animatedTime = 0;
    if (animated) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if (_rootViewController.childViewControllers.count>1) {
        return NO;
    }
...
- (void)pan:(UIPanGestureRecognizer*)pan{
    if (_panGestureRecognizer.state==UIGestureRecognizerStateBegan) {
        ...
        if(velocity.x>0){
            if (_currentView.frame.origin.x>=0 && _leftViewController && !_leftViewController.view.superview) {
                //发送向右滑动的消息
                [[NSNotificationCenter defaultCenter] postNotificationName:@"Notification_SHOWLEFTVIEW" object:nil userInfo:nil];
                [self willShowLeftViewController];
            }
        }else if (velocity.x<0) {
            if (_currentView.frame.origin.x<=0 && _rightViewController && !_rightViewController.view.superview) {
                //发送向左滑动的消息
                [[NSNotificationCenter defaultCenter] postNotificationName:@"Notification_SHOWFILTERVIEW" object:nil userInfo:nil];
                [self willShowRightViewController];
    ...
    if (xoffset>0&&(self.sliderModel == MBSlideModelRightAndLeft || self.sliderModel == MBSlideModelRight)) {//向右滑
        if (_leftViewController && _leftViewController.view.superview) {
    ...
    }else if(xoffset<0&&(self.sliderModel == MBSlideModelLeft || self.sliderModel == MBSlideModelRightAndLeft)){//向左滑
        if (_rightViewController && _rightViewController.view.superview) {
