...
static CGFloat const KVNCircleProgressViewToStatusLabelVerticalSpaceConstraintConstant = 15.0f;
...
static CGFloat const KVNContentViewCornerRadius = 3.0f;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
	if (self = [super initWithCoder:aDecoder]) {
		// Appearance
		_backgroundFillColor = [UIColor colorWithWhite:1.0f alpha:1.0f];
		_backgroundTintColor = [UIColor clearColor];
		
		_circleStrokeForegroundColor = [UIColor whiteColor];
		_circleStrokeBackgroundColor = [_circleStrokeForegroundColor colorWithAlphaComponent:1.0f];
		_circleFillBackgroundColor = [UIColor clearColor];
		
		_successColor = [UIColor whiteColor];
		_errorColor = [UIColor whiteColor];
				...
				view:(UIView *)superview
{
	...
	_statusColor = fullScreen? [UIColor blackColor]: [UIColor whiteColor];
    _successColor = fullScreen? [UIColor blackColor]: [UIColor whiteColor];
    _errorColor = fullScreen? [UIColor blackColor]: [UIColor whiteColor];
...
- (void)setupConstraints
{
	...
	self.statusLabelBottomToSuperViewConstraint.constant = statusInset + 10;
...
- (void)setupProgressCircle
{
	...
	self.circleBackgroundLineLayer.lineWidth = self.lineWidth * 2;
...
- (void)setupSuccessUI
{
	...
	self.checkmarkLayer.lineWidth = self.lineWidth * 2;
...
- (void)setupErrorUI
{
	...
	[crossPath moveToPoint:CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * endFloat, CGRectGetHeight(self.circleProgressView.bounds) * beginFloat)];
	[crossPath addLineToPoint:CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * beginFloat, CGRectGetHeight(self.circleProgressView.bounds) * endFloat)];
	[crossPath moveToPoint:CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * beginFloat, CGRectGetHeight(self.circleProgressView.bounds) * beginFloat)];
	[crossPath addLineToPoint:CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * endFloat, CGRectGetHeight(self.circleProgressView.bounds) * endFloat)];
	...
	self.crossLayer.lineWidth = self.lineWidth * 2;
...
- (void)setupStatus:(NSString *)status
{
    ...
    _statusColor = [self isFullScreen]? [UIColor blackColor]: [UIColor whiteColor];
...
- (CGFloat)circleSize
{
	...
	if (_circleSize == 0) {
		_circleSize = ([self isFullScreen]) ? 34.0f : 30.0f;
	}
