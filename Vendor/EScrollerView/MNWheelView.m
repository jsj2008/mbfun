...
//    int mid=count;
    ...
    for (int i=0; i<count; i++) {
        ...
        else
        {
            ...
            _index=(int)view1.tag;
        }
        view1.layer.masksToBounds=YES;
        view1.layer.cornerRadius=3.0;
        [view1 sd_setImageWithURL:[NSURL URLWithString:images[i]] placeholderImage:[UIImage imageNamed:DEFAULT_LOADING_IMAGE]];
        
   
        //view1.clipsToBounds  = YES;
        //添加四个边阴影
//        view1.layer.shadowColor = [UIColor blackColor].CGColor;//阴影颜色
        view1.layer.shadowColor =[Utils HexColor:0X3B3B3B Alpha:1].CGColor;
...
{
    if (btn.tag==3) {
        NSArray *views = [_baseView subviews];
        for (UIImageView* imgView in views) {
            if (imgView.tag ==_index) {
          
                  _click(_index,imgView.image);
            }
        }
