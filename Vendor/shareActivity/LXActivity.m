...
//    self.backGroundView.layer.cornerRadius = CORNER_RADIUS;
    self.backGroundView.backgroundColor = [UIColor whiteColor];//ACTIONSHEET_BACKGROUNDCOLOR;
        ...
        if (shareButtonImagesNameArray.count > 0) {
            ...
            for (int i = 1; i < shareButtonImagesNameArray.count+1; i++) {
                ...
                if (self.isHadTitle == YES) {
                    [shareButton setFrame:CGRectMake(SHAREBUTTON_INTERVAL_WIDTH+((line-1)*(SHAREBUTTON_INTERVAL_WIDTH+SHAREBUTTON_WIDTH)-20), self.LXActivityHeight+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)-10), SHAREBUTTON_WIDTH, SHAREBUTTON_HEIGHT)];
                }
                else{
                    [shareButton setFrame:CGRectMake(SHAREBUTTON_INTERVAL_WIDTH+((line-1)*(SHAREBUTTON_INTERVAL_WIDTH+SHAREBUTTON_WIDTH)-20), SHAREBUTTON_INTERVAL_HEIGHT+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)-10), SHAREBUTTON_WIDTH, SHAREBUTTON_HEIGHT)];
                }
...
//    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(20, 10, SCREEN_WIDTH-40, self.LXActivityHeight)];
...
//    [self.backGroundView addSubview:view];
    ...
    if (cancelButtonTitle) {
        ...
        if (self.isHadTitle == YES || self.isHadShareButton == YES) {
            [cancelButton setFrame:CGRectMake(cancelButton.frame.origin.x-20, self.LXActivityHeight+5, cancelButton.frame.size.width, cancelButton.frame.size.height)];
    ...
    [UIView animateWithDuration:ANIMATE_DURATION animations:^{
        [self.backGroundView setFrame:CGRectMake(20, [UIScreen mainScreen].bounds.size.height-self.LXActivityHeight, [UIScreen mainScreen].bounds.size.width-40, self.LXActivityHeight-BUTTON_HEIGHT-25)];
...
{
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH-20, BUTTON_INTERVAL_HEIGHT, BUTTON_WIDTH+40, BUTTON_HEIGHT)];
...
//    cancelButton.layer.borderColor = BUTTON_BORDER_COLOR;
    ...
    UIImage *image = [UIImage imageWithColor:[UIColor whiteColor]];//CANCEL_BUTTON_COLOR
...
{
    UILabel *shareLabel = [[UILabel alloc] initWithFrame:CGRectMake(SHARETITLE_INTERVAL_WIDTH+((line-1)*(SHARETITLE_INTERVAL_WIDTH+SHARETITLE_WIDTH)-20), SHARETITLE_INTERVAL_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)-10), SHARETITLE_WIDTH, SHARETITLE_HEIGHT)];
