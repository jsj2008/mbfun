...
- (void)adjustStateWithContentOffset
{
    ...
    if (currentOffsetY + self.scrollViewOriginalInset.top >= -self.mj_height && currentOffsetY + self.scrollViewOriginalInset.top < - 20) {
        int index = fabs(self.mj_height + currentOffsetY + 20)/ ((self.mj_height)/ 5);
...
- (void)setCustomizeState:(MJRefreshState)customizeState{
    // 1.一样的就直接返回
    if (self.state == customizeState) return;
    
    // 2.保存旧状态
    MJRefreshState oldState = self.state;
    
    // 3.调用父类方法
    [super setCustomizeState:customizeState];
    ...
    switch (customizeState) {
        case MJRefreshStateNormal: // 下拉可以刷新
        {
            ...
            if (MJRefreshStateRefreshing == oldState) {
                ...
                [UIView animateWithDuration:MJRefreshSlowAnimationDuration animations:^{
                    //#warning 这句代码修复了，top值不断累加的bug
                    self.scrollView.mj_contentOffsetY = _contentOffSetY;
                    self.scrollView.mj_contentInsetTop = self.scrollViewOriginalInset.top;
                }];
            } else {
                // 执行动画
        ...
        {
            // 执行动画
            _contentOffSetY = self.scrollView.contentOffset.y;
            [UIView animateWithDuration:MJRefreshFastAnimationDuration animations:^{
                // 1.增加滚动区域
                CGFloat top = self.scrollViewOriginalInset.top + self.mj_height;
                self.scrollView.mj_contentInsetTop = top;
                ...
                // 2.设置滚动位置
                self.scrollView.mj_contentOffsetY = _contentOffSetY - top;
            }];
            break;
        }
