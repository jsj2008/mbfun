...
- (void)addHeaderJumpWithTarget:(id)target action:(SEL)opration titleName:(NSString *)titleName{
    // 1.创建新的footer
    if (!self.header) {
        MJRefreshHeaderView *header = [MJRefreshHeaderView header];
    ...
    // 2.设置目标和回调方法
    self.header.isShowJump = YES;
    self.header.beginRefreshingTaget = target;
    self.header.beginRefreshingAction = opration;
    self.header.pullToRefreshText = titleName;
    self.header.releaseToRefreshText = titleName;
    self.header.refreshingText = titleName;
}
...
- (void)headerCustomizeBeginRefreshing{
    [self.header customizeBeginRefreshing];
}
...
{
    [self addFooterAutoRefreshWithTarget:target action:action];
    return;
...
- (void)addFooterAutoRefreshWithTarget:(id)target action:(SEL)action{
    // 1.创建新的footer
    if (!self.footer) {
        MJRefreshFooterView *footer = [MJRefreshFooterView footer];
        [self addSubview:footer];
    ...
    self.footer.beginRefreshingTaget = target;
    self.footer.beginRefreshingAction = action;
    self.footer.isAutoRefresh = YES;
}

- (void)addFooterJumpWithTarget:(id)target action:(SEL)opration titleName:(NSString *)titleName{
    // 1.创建新的footer
    if (!self.footer) {
        MJRefreshFooterView *footer = [MJRefreshFooterView footer];
        footer.arrowImage.hidden = YES;
        [self addSubview:footer];
        self.footer = footer;
    }
    
    // 2.设置目标和回调方法
    self.footer.isShowJump = YES;
    self.footer.beginRefreshingTaget = target;
    self.footer.beginRefreshingAction = opration;
    self.footer.pullToRefreshText = titleName;
    self.footer.releaseToRefreshText = titleName;
    self.footer.refreshingText = titleName;
}

- (void)adjustFrameWithContentSize{
    [self.footer adjustFrameWithContentSize];
    [self.footer adjustStateWithContentOffset];
}
