...
- (void)adjustStateWithContentOffset
{
    ...
    if (_isAutoRefresh && currentOffsetY >= happenOffsetY - 150) {
        self.state = MJRefreshStateRefreshing;
        return;
    }else if (currentOffsetY <= happenOffsetY) return;
    
    if (self.isShowJump) {
        CGAffineTransform transform = CGAffineTransformMakeRotation(M_PI);
        if (currentOffsetY + self.scrollViewOriginalInset.bottom - happenOffsetY <= 64){
        ...
        [self showPullAction:currentOffsetY happenOffsetY:happenOffsetY];
    }
...
- (void)showPullAction:(CGFloat)currentOffsetY happenOffsetY:(CGFloat)happenOffsetY{
    if (currentOffsetY + self.scrollViewOriginalInset.bottom - happenOffsetY <= 64 && currentOffsetY + self.scrollViewOriginalInset.bottom >= 20) {
        int index = fabs(currentOffsetY + self.scrollViewOriginalInset.bottom - happenOffsetY - 20)/ ((self.mj_height)/ 5);
        NSString *nameString = [NSString stringWithFormat:@"Unico/fun_loading_%d", index % 4];
        self.arrowImage.image = [UIImage imageNamed:nameString];
    }else{
        self.arrowImage.image = [UIImage imageNamed:@"Unico/fun_loading_0"];
    }
}
...
{
    if (_isAutoRefresh && self.scrollView.contentSize.height < self.scrollView.height + 150){
        _isAutoRefresh = NO;
        return;
    }
    // 1.一样的就直接返回
    ...
    if (state == MJRefreshStateRefreshing) {
        if (self.contentHeight == self.scrollView.contentSize.height && _isAutoRefresh) {
            _isAutoRefresh = NO;
            return;
        }
        _isAutoRefresh = YES;
