...
- (UILabel *)statusLabel
{
    if (!_statusLabel) {
        ...
        //隐藏掉的提示语
        _statusLabel.hidden = !_isShowJump;
    }
    return _statusLabel;
}
...
- (UIImageView *)arrowImage
{
    ...
    if (_isShowJump) {
        ...
        arrowImageName = @"Unico/icon_home_pull";
    }else{
        arrowImageName = @"Unico/fun_loading_0";
    }
    if (!_arrowImage) {
        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:arrowImageName]];//MJRefreshSrcName(@"arrow.png")
...
- (void)setIsShowJump:(BOOL)isShowJump{
    _isShowJump = isShowJump;
    _statusLabel.hidden = !_isShowJump;
    if (isShowJump) {
        _arrowImage.image = [UIImage imageNamed:@"Unico/icon_home_pull"];
    }else{
        _arrowImage.image = [UIImage imageNamed:@"Unico/fun_loading_0"];
    }
}
...
- (void)layoutSubviews
{
    ...
    if (_isShowJump) {
        arrowX = self.mj_width * 0.5 - 100;
    }
    self.arrowImage.center = CGPointMake(arrowX, self.mj_height * 0.5 - (_isShowJump? 0: 5));
...
- (void)customizeBeginRefreshing{
    if (self.state == MJRefreshStateRefreshing) {
        ...
        if (self.window) {
            self.customizeState = MJRefreshStateRefreshing;
        } else {
            //    #warning 不能调用set方法
            _state = MJRefreshStateWillRefreshing;
            _customizeState = MJRefreshStateWillRefreshing;
            [super setNeedsDisplay];
        }
    }
}
...
{
    // 返回状态
    if ([self.beginRefreshingTaget respondsToSelector:@selector(returRefreshingnState:)]) {
        objc_msgSend(self.beginRefreshingTaget, sel_getUid("returRefreshingnState:"), self.state);
    }
...
- (void)startRefreshAnimation{
    _showImageIdenx = 0;
    _timer = [NSTimer timerWithTimeInterval:0.15 target:self selector:@selector(showRefreshAnimation) userInfo:nil repeats:YES];
    [[NSRunLoop mainRunLoop]addTimer:_timer forMode:NSRunLoopCommonModes];
    [_timer fire];
}

- (void)showRefreshAnimation{
    NSString *nameString = [NSString stringWithFormat:@"Unico/fun_loading_%d", _showImageIdenx% 4];
    _arrowImage.image = [UIImage imageNamed:nameString];
    _showImageIdenx ++;
}
