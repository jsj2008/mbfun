...
//
{
    ...
    if (remoteHostName!=nil)
        [remoteHostName release];
    remoteHostName = [[NSString alloc] initWithFormat:@"%@",hostName];
    remotePort = port;
...
//
{
    ...
    return [[[NSString alloc] initWithUTF8String:inet_ntoa(*address)] autorelease];
}
...
//
{
    ...
    printf("===================\n");
    printf(">>>AbsSock readData=%s,port=%d,[data length]=%d\n>>>",[remoteHostName UTF8String],remotePort, readBufferSize);
    for(int i=0;i<readBufferSize;i++)
    {
        printf("%0.2x ",((unsigned char*)readBuffer)[i]);
    }
    printf("\n");
...
- (void)writeData:(Byte *)bytes Length:(int)len
{
    int sent;
...
//
{
    ...
    int len = [data length];
    [self writeData:(Byte *)bytes Length:len];
}
