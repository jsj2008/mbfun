...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	...
	if ((self = [super init]))
	{
		...
		if (IS_IOS5_LATER) dispatch_queue_set_specific(socketQueue, IsOnSocketQueueOrTargetQueueKey, nonNullUnusedPointer, NULL);
...
- (void)dealloc
{
	...
	if ((!IS_IOS5_LATER && dispatch_get_current_queue() == socketQueue) || dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
{
	NSAssert((!IS_IOS5_LATER && dispatch_get_current_queue() == socketQueue) || dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey), @"Must be dispatched on socketQueue");
...
{
	void *nonNullUnusedPointer = (__bridge void *)self;
	if (IS_IOS5_LATER) dispatch_queue_set_specific(socketNewTargetQueue, IsOnSocketQueueOrTargetQueueKey, nonNullUnusedPointer, NULL);
}
...
{
	if (IS_IOS5_LATER) dispatch_queue_set_specific(socketOldTargetQueue, IsOnSocketQueueOrTargetQueueKey, NULL, NULL);
}
